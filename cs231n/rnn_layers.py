...
def rnn_step_backward(dnext_h, cache):
    ...
    dprev_h = dout.dot(Wh.T) # note: transpose on Wh was not necessary for dimensions to match
    dWh     = dout.T.dot(prev_h).T # note: final transpose was not necessary for dimensions to match
    dx      = dout.dot(Wx.T)
    dWx     = dout.T.dot(x).T    
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for cnt in range(T):
        xStep = x[:,cnt,:]
        if cnt == 0:
            next_h, cacheTemp = rnn_step_forward(xStep, h0, Wx, Wh, b)
        else:
            next_h, cacheTemp = rnn_step_forward(xStep, next_h, Wx, Wh, b)
        h[:,cnt,:] = next_h
...
def rnn_backward(dh, cache):
    ...
    for cnt in range(T-1,-1,-1):
        ...
        if cnt == T-1: dprev_h = 0
        dxTmp, dprev_h, dWxTmp, dWhTmp, dbTmp = rnn_step_backward(dh[:,cnt,:] + dprev_h, cache.pop())
...
def word_embedding_backward(dout, cache):
    ...
    xflat,V = cache
    dW = np.zeros((V,D))
    np.add.at(dW, xflat, dout.reshape((N*T,-1)) )
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return dW
